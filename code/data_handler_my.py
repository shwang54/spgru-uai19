...
class BouncingMNISTDataHandler(object):
  ...
  # -----------------------------------------------------------------------------------------------------------------
  def GetControlledTrajectory(self, batch_size, my_angle, my_x, my_y, delta_angle, my_speed):
    length = self.seq_length_
    canvas_size = self.image_size_ - self.digit_size_

    # Initial position uniform random inside the box.
    # set initial positions of entire batch
    y = np.full(batch_size, my_y)
    if np.size(my_x) > 1:
      x = my_x
    else:
      x = np.full(batch_size, my_x)

    # Choose a random velocity.
    # Use my angle in degrees into theta for the entire batch
    theta = np.full(batch_size, my_angle) / 180.0 * np.pi
    v_y = np.sin(theta)
    v_x = np.cos(theta)
  ...
  # -----------------------------------------------------------------------------------------------------------------
  # --------------------- GET TEST BATCH WITH CONTROLLED TRAJECTORIES
  # -----------------------------------------------------------------------------------------------------------------
  def GetControlledTestBatch(self, my_angle, my_x, my_y, delta_angle, my_speed, verbose=False):
    start_y, start_x = self.GetControlledTrajectory(self.batch_size_ * self.num_digits_, my_angle=my_angle, my_x=my_x, my_y=my_y, delta_angle=delta_angle, my_speed=my_speed)
    ...
    for j in xrange(self.batch_size_):
      for n in xrange(self.num_digits_):
        ...
        ind = self.test_indices_[self.test_row_]
        self.test_row_ += 1

        # Only if we used all preset number of train cases. Doesn't really matter.
        if self.test_row_ == self.data_.shape[0]:
          self.test_row_ = 0
...
class VideoPatchDataHandler(object):
  ...
  def GetBatch(self, verbose=False):
    # -----------------------------------------------------------------------------------------------------------------
    # ------------------------------------------------------ reset row index when data_size not divisible by batch_size
    # -----------------------------------------------------------------------------------------------------------------
    if (self.row_+self.batch_size_) >  self.data_.shape[0]:
      self.row_ = 0
  ...
  def GetBatchByIndex(self, index, verbose=False):
    # -----------------------------------------------------------------------------------------------------------------
    # ------------------------------------------------------ reset row index when data_size not divisible by batch_size
    # -----------------------------------------------------------------------------------------------------------------
    if (self.row_+index) >  self.data_.shape[0]:
        index_remainder = (self.row_+index)-self.data_.shape[0]
        index = self.data_.shape[0]-self.row_
        idx1 = np.arange(self.row_,self.row_+index)
        idx2 = np.arange(0,index_remainder)
        idx = np.concatenate((idx1, idx2))
        minibatch = self.data_[idx]
        print minibatch[0].shape
    else:
        minibatch = self.data_[self.row_:self.row_+index]

    self.row_ = self.row_ + index
